...
class Enumeration(GenericEnumeration):
    def getPrivileges(self, *args, **kwargs):
        warnMsg = "在Microsoft SQL Server上 "
        warnMsg += "无法获取数据库用户权限 "
        warnMsg += "sqlmap将检查数据库用户是否为数据库管理员"
    ...
    def searchColumn(self):
        ...
        for column in colList:
            ...
            if conf.tbl:
                ...
                infoMsgTbl = " 表 %s '%s'" % ("s" if len(_) > 1 else "", ", ".join(tbl for tbl in _))
            ...
            if conf.db:
                _ = conf.db.split(',')
                infoMsgDb = " 数据库 %s '%s'" % ("s" if len(_) > 1 else "", ", ".join(db for db in _))
            elif conf.excludeSysDbs:
                infoMsgDb = " 不在系统数据库中 %s '%s'" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(db for db in self.excludeDbsList))
            else:
                infoMsgDb = " 跨所有数据库"
