...
class Enumeration(GenericEnumeration):
    ...
    def getPrivileges(self, *args, **kwargs):
        warnMsg = "在Sybase上，无法获取 "
        warnMsg += "数据库用户权限，sqlmap将检查 "
        warnMsg += "数据库用户是否为数据库管理员"
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                errMsg = "无法检索数据库 "
                errMsg += "上的表 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
        ...
        for tbl in tblList:
            if conf.db is not None and len(kb.data.cachedColumns) > 0 \
               ...
               kb.data.cachedColumns[conf.db]:
                infoMsg = "在 "
                infoMsg += "数据库'%s'上提取表的列 " % unsafeSQLIdentificatorNaming(conf.db)
                logger.info(infoMsg)
    ...
    def searchDb(self):
        warnMsg = "在Sybase上未实现数据库搜索"
        logger.warning(warnMsg)

        return []

    def searchTable(self):
        warnMsg = "在Sybase上未实现表搜索"
        logger.warning(warnMsg)

        return []

    def searchColumn(self):
        warnMsg = "在Sybase上未实现列搜索"
        logger.warning(warnMsg)

        return []

    def search(self):
        warnMsg = "在Sybase上搜索选项不可用"
        logger.warning(warnMsg)

    def getHostname(self):
        warnMsg = "在Sybase上，无法枚举主机名"
        logger.warning(warnMsg)

    def getStatements(self):
        warnMsg = "在Sybase上，无法枚举SQL语句"
        logger.warning(warnMsg)
