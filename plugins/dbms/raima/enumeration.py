...
class Enumeration(GenericEnumeration):
    def getBanner(self):
        warnMsg = "在Raima数据库管理器上，无法获取横幅"
        logger.warning(warnMsg)

        return None

    def getCurrentUser(self):
        warnMsg = "在Raima数据库管理器上，无法枚举当前用户"
        logger.warning(warnMsg)

    def getCurrentDb(self):
        warnMsg = "在Raima数据库管理器上，无法获取当前数据库的名称"
        logger.warning(warnMsg)

    def isDba(self, user=None):
        warnMsg = "在Raima数据库管理器上，无法测试当前用户是否为DBA"
        logger.warning(warnMsg)

    def getUsers(self):
        warnMsg = "在Raima数据库管理器上，无法枚举用户"
        logger.warning(warnMsg)
