...
class Search(object):
    ...
    def searchDb(self):
        ...
        for db in dbList:
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                if not isNumPosStrValue(count):
                    ...
                    if dbConsider == "1":
                        warnMsg += "s LIKE"
                    warnMsg += " '%s' 找到" % unsafeSQLIdentificatorNaming(db)
        ...
        conf.dumper.lister("找到数据库", foundDbs)

    def searchTable(self):
        bruteForce = False

        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            errMsg = "information_schema不可用, "
            errMsg += "后端DBMS为MySQL<5.0"
            bruteForce = True
        ...
        for tbl in tblList:
            ...
            if dbCond and conf.db:
                ...
                infoMsg += " 数据库 %s '%s'" % ("s" if len(_) > 1 else "", ", ".join(db for db in _))
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for db in foundTbls:
                    ...
                    if tblConsider == "1":
                        infoMsg += "s LIKE"
                    infoMsg += " '%s' 数据库 '%s'" % (unsafeSQLIdentificatorNaming(tbl), unsafeSQLIdentificatorNaming(db))
                    logger.info(infoMsg)
        ...
        for column in colList:
            ...
            if tblCond:
                if conf.tbl:
                    ...
                    infoMsgTbl = " 表 %s '%s'" % ("s" if len(tbls) > 1 else "", ", ".join(unsafeSQLIdentificatorNaming(tbl) for tbl in tbls))
            ...
            if dbCond:
                if conf.db:
                    _ = conf.db.split(',')
                    whereDbsQuery = " AND (" + " OR ".join("%s = '%s'" % (dbCond, unsafeSQLIdentificatorNaming(db)) for db in _) + ")"
                    infoMsgDb = " 数据库 %s '%s'" % ("s" if len(_) > 1 else "", ", ".join(unsafeSQLIdentificatorNaming(db) for db in _))
                elif conf.excludeSysDbs:
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for column, dbData in foundCols.items():
                    ...
                    for db in dbData:
                        ...
                        if colConsider == "1":
                            infoMsg += "s LIKE"
                        infoMsg += " '%s' 数据库 '%s'" % (unsafeSQLIdentificatorNaming(column), unsafeSQLIdentificatorNaming(db))
                        logger.info(infoMsg)
