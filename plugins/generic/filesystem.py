...
class Filesystem(object):
    ...
    def _checkFileLength(self, localFile, remoteFile, fileRead=False):
        ...
        try:
            ...
            warnMsg = "缺少文件 '%s'" % localFile
            logger.warning(warnMsg)
            localFileSize = 0

        if fileRead and Backend.isDbms(DBMS.PGSQL):
            logger.info("无法在PostgreSQL上检查读取文件 '%s' 的长度" % remoteFile)
            sameFile = True
        else:
            logger.debug("检查远程文件的长度 '%s'" % remoteFile)
            ...
            if isNumPosStrValue(remoteFileSize):
                ...
                if localFileSize == remoteFileSize:
                    ...
                    infoMsg += "'%s' 等于 (%d B)" % (remoteFile, localFileSize)
                elif remoteFileSize > localFileSize:
                    infoMsg = "远程文件'%s' 大于 (%d B) " % (remoteFile, remoteFileSize)
                    infoMsg += "本地文件 '%s' (%dB)" % (localFile, localFileSize)
                else:
                    infoMsg = "远程文件 '%s' 小于 (%d B) " % (remoteFile, remoteFileSize)
                    infoMsg += "文件 '%s' (%d B)" % (localFile, localFileSize)

                logger.info(infoMsg)
            else:
                sameFile = False
                warnMsg = "看起来文件尚未写入（通常 "
                warnMsg += "通常发生在DBMS进程用户在 "
                warnMsg += "目标路径中没有写入权限的情况下)"
                logger.warning(warnMsg)
    ...
    def askCheckReadFile(self, localFile, remoteFile):
        if not kb.bruteMode:
            message = "是否要确认远程文件'%s' " % remoteFile
            message += "已从后端成功下载 "
            message += "DBMS文件系统? [Y/n] "
    ...
    def nonStackedReadFile(self, remoteFile):
        errMsg = "'必须定义'nonStackedReadFile'方法"
        errMsg += "到特定的DBMS插件"
        raise SqlmapUndefinedMethod(errMsg)

    def stackedReadFile(self, remoteFile):
        errMsg = "'必须定义'stackedReadFile'方法 "
        errMsg += "到特定的DBMS插件"
        raise SqlmapUndefinedMethod(errMsg)

    def unionWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = "必须定义'unionWriteFile'方法 "
        errMsg += "到特定的DBMS插件"
        raise SqlmapUndefinedMethod(errMsg)

    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = "必须定义'stackedWriteFile'方法 "
        errMsg += "到特定的DBMS插件"
        raise SqlmapUndefinedMethod(errMsg)
