...
class Custom(object):
    ...
    def sqlQuery(self, query):
        ...
        try:
            ...
            if not re.search(r"\b(OPENROWSET|INTO)\b", query, re.I) and (not sqlType or "SELECT" in sqlType):
                infoMsg = "正在获取 %s 查询输出: '%s'" % (sqlType if sqlType is not None else "SQL", query)
                logger.info(infoMsg)
                ...
                warnMsg = "只有在支持堆叠查询时， "
                warnMsg += "才能执行非查询SQL语句"
                logger.warning(warnMsg)

                return None
            else:
                if sqlType:
                    infoMsg = "正在执行 %s 语句: '%s'" % (sqlType if sqlType is not None else "SQL", query)
                else:
                    infoMsg = "正在执行未知SQL命令: '%s'" % query
                logger.info(infoMsg)
    ...
    def sqlShell(self):
        infoMsg = "正在调用 %s外壳." % Backend.getIdentifiedDbms()
        infoMsg += "要退出，请键入'x'或'q'，然后按ENTER键"
        logger.info(infoMsg)
