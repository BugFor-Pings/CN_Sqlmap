...
class Databases(object):
    ...
    def getCurrentDb(self):
        ...
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2, DBMS.PGSQL, DBMS.MONETDB, DBMS.DERBY, DBMS.VERTICA, DBMS.PRESTO, DBMS.MIMERSQL, DBMS.CRATEDB, DBMS.CACHE, DBMS.FRONTBASE):
            warnMsg = "在 %s上，您需要" % Backend.getIdentifiedDbms()
            warnMsg += "将枚举的架构名称用作其他DBMS上 "
            warnMsg += "数据库名称的对应项"
            singleTimeWarnMessage(warnMsg)
        elif Backend.getIdentifiedDbms() in (DBMS.ALTIBASE, DBMS.CUBRID):
            warnMsg = "在 %s上,您需要使用" % Backend.getIdentifiedDbms()
            warnMsg += "枚举用户名作为其他DBMS上 "
            warnMsg += "数据库名称的对应项"
            singleTimeWarnMessage(warnMsg)
    ...
    def getDbs(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            warnMsg = "information_schema不可用, "
            warnMsg += "端DBMS是MySQL<5. "
            warnMsg += "数据库名称将从'mysql'数据库中获取"
            ...
            warnMsg = "架构名称将在 %s " % Backend.getIdentifiedDbms()
            warnMsg += "用于枚举，"
            warnMsg += "作为其他DBMS上数据库名称的对应"
            logger.warning(warnMsg)

            infoMsg = "获取数据库（架构）名称"

        elif Backend.getIdentifiedDbms() in (DBMS.ALTIBASE, DBMS.CUBRID):
            warnMsg = "用户名将在 %s " % Backend.getIdentifiedDbms()
            warnMsg += "上用于枚举，作为其他 "
            warnMsg += "DBMS上数据库名称的对应项"
            logger.warning(warnMsg)
    ...
    def getTables(self, bruteForce=None):
        ...
        if bruteForce is None:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                ...
                if not tables:
                    warnMsg = "无法检索表名, "
                    warnMsg += "后端DBMS是 %s" % Backend.getIdentifiedDbms()
                    logger.warning(warnMsg)
                    bruteForce = True
                else:
                    return tables
        ...
        if bruteForce:
            ...
            message = "是否要使用公共表存在性检查？ %s " % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
                        ...
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                ...
                                if not isNoneValue(comment):
                                    infoMsg = "已检索表'%s' 的注释 '%s'" % (comment, unsafeSQLIdentificatorNaming(table))
                                    if METADB_SUFFIX not in db:
                                        infoMsg += " 在数据库 '%s'" % unsafeSQLIdentificatorNaming(db)
                                    logger.info(infoMsg)
                            else:
                                warnMsg = "在 %s上 " % Backend.getIdentifiedDbms()
                                warnMsg += "无法获取表注释"
                                singleTimeWarnMessage(warnMsg)
        ...
        if not kb.data.cachedTables and isInferenceAvailable() and not conf.direct:
            for db in dbs:
                ...
                for _query, _count in ((rootQuery.blind.query, rootQuery.blind.count), (getattr(rootQuery.blind, "query2", None), getattr(rootQuery.blind, "count2", None))):
                    ...
                    infoMsg = "获取数据库的 "
                    infoMsg += "表数'%s'" % unsafeSQLIdentificatorNaming(db)
                    logger.info(infoMsg)
                    ...
                    if count == 0:
                        warnMsg = "数据库 '%s' " % unsafeSQLIdentificatorNaming(db)
                        warnMsg += "似乎为空"
                        logger.warning(warnMsg)
                        break
                        ...
                        warnMsg = "无法检索数据库 "
                        warnMsg += "的表数 '%s'" % unsafeSQLIdentificatorNaming(db)
                        singleTimeWarnMessage(warnMsg)
                        continue
                    ...
                    if tables:
                        ...
                        if conf.getComments:
                            for table in tables:
                                ...
                                if hasattr(_, "query"):
                                    ...
                                    if not isNoneValue(comment):
                                        infoMsg = "检索到表 '%s' 的注释'%s'" % (comment, unsafeSQLIdentificatorNaming(table))
                                        if METADB_SUFFIX not in db:
                                            infoMsg += " 在数据库 '%s'" % unsafeSQLIdentificatorNaming(db)
                                        logger.info(infoMsg)
                                else:
                                    warnMsg = "在 %s 上不是 " % Backend.getIdentifiedDbms()
                                    warnMsg += "可以获取表注释"
                                    singleTimeWarnMessage(warnMsg)
                        ...
                        warnMsg = "无法检索数据库的 "
                        warnMsg += "表名'%s'" % unsafeSQLIdentificatorNaming(db)
                        logger.warning(warnMsg)
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                if METADB_SUFFIX not in conf.db:
                    errMsg += " 在数据库 '%s'中" % unsafeSQLIdentificatorNaming(conf.db)
                raise SqlmapNoneDataException(errMsg)
            else:
        ...
        if bruteForce:
            ...
            if kb.choices.columnExists is None:
                message = "是否要使用公共列存在性检查? %s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
        ...
        if any(isTechniqueAvailable(_) for _ in (PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)) or conf.direct:
            for tbl in tblList:
                if conf.db is not None and len(kb.data.cachedColumns) > 0 \
                   ...
                   kb.data.cachedColumns[conf.db]:
                    infoMsg = "已从中提取表列 "
                    infoMsg += "数据库 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                ...
                if dumpMode and colList:
                    ...
                    infoMsg += "用于表 '%s' " % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX not in conf.db:
                            ...
                            if name:
                                if conf.getComments:
                                    ...
                                    if hasattr(_, "query"):
                                        ...
                                        comment = unArrayizeValue(inject.getValue(query, blind=False, time=False))
                                        if not isNoneValue(comment):
                                            infoMsg = "已检索到列'%s' 的注释 '%s'" % (comment, name)
                                            logger.info(infoMsg)
                                    else:
                                        warnMsg = "在 %s上不是 " % Backend.getIdentifiedDbms()
                                        warnMsg += "可以获取列注释"
                                        singleTimeWarnMessage(warnMsg)
            ...
            for tbl in tblList:
                if conf.db is not None and len(kb.data.cachedColumns) > 0 \
                   ...
                   kb.data.cachedColumns[conf.db]:
                    infoMsg = "从数据库中提取"
                    infoMsg += "表列 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                ...
                if dumpMode and colList:
                    ...
                    infoMsg += "用于表 '%s' " % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX not in conf.db:
                        infoMsg += "在数据库中 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        if not columns:
                            errMsg = "无法检索表%s " % ("number of " if not Backend.isDbms(DBMS.MSSQL) else "")
                            errMsg += "的'%s列' " % unsafeSQLIdentificatorNaming(tbl)
                            if METADB_SUFFIX not in conf.db:
                                errMsg += "在数据库里 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
                            logger.error(errMsg)
                            continue
                    ...
                    if not isNoneValue(column):
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                ...
                                comment = unArrayizeValue(inject.getValue(query, union=False, error=False))
                                if not isNoneValue(comment):
                                    infoMsg = "已检索到列 '%s' (在%s上）'%s'" % (comment, column)
                                    logger.info(infoMsg)
                            else:
                                warnMsg = "的注释 %s , " % Backend.getIdentifiedDbms()
                                warnMsg += "无法获取列注释"
                                singleTimeWarnMessage(warnMsg)
        ...
        if not kb.data.cachedColumns:
            warnMsg = "无法检索表的列名 "
            warnMsg += (" '%s' " % unsafeSQLIdentificatorNaming(unArrayizeValue(tblList))) if len(tblList) == 1 else "any table "
            if METADB_SUFFIX not in conf.db:
                warnMsg += "在数据库中 '%s'" % unsafeSQLIdentificatorNaming(conf.db)
            logger.warning(warnMsg)
    ...
    def getCount(self):
        if not conf.tbl:
            warnMsg = "缺少表参数 "
            warnMsg += "sqlmap将检索所有数据库 "
            warnMsg += "管理系统数据库表的条目数"
            logger.warning(warnMsg)
        ...
        if conf.tbl is not None and conf.db is None and Backend.getIdentifiedDbms() not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD, DBMS.MCKOI, DBMS.EXTREMEDB):
            warnMsg = "缺少数据库参数, "
            warnMsg += "sqlmap将使用当前数据库 "
            warnMsg += "检索表'%s'的条目数" % unsafeSQLIdentificatorNaming(conf.tbl)
            logger.warning(warnMsg)
