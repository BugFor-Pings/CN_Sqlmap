...
class Users(object):
    ...
    def getPasswordHashes(self):
        ...
        if not kb.data.cachedUsersPasswords:
            ...
            message = "是否要对检索到的密码哈希 "
            message += "执行基于字典的攻击？ [Y/n/q]"
            ...
            for user in users:
                ...
                if Backend.isDbms(DBMS.INFORMIX):
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        warnMsg = "无法检索用户的'%s' "% outuser
                        warnMsg += "权限数 "
                        logger.warning(warnMsg)
                        continue

                infoMsg = "获取用户的权限 '%s'" % outuser
                logger.info(infoMsg)
    ...
    def getRoles(self, query2=False):
        warnMsg = "在 %s 上，角色的概念不 " % Backend.getIdentifiedDbms()
        warnMsg += "存在sqlmap将枚举特权"
        logger.warning(warnMsg)
