...
def action():
    ...
    if not Backend.getDbms() or not conf.dbmsHandler:
        ...
        if htmlParsed:
            errMsg += ", 但从HTML错误页面中可以看到 "
            errMsg += "可以确定 "
            errMsg += "后端DBMS是 %s" % htmlParsed

        if htmlParsed and htmlParsed.lower() in SUPPORTED_DBMS:
            errMsg += ". 不要手动指定后端DBMS, "
            errMsg += "sqlmap将为您识别DBMS"
        elif kb.nullConnection:
            errMsg += ". 您可以尝试在不使用优化的情况下重新运行 "
            errMsg += "转换 '%s'" % ("-o" if conf.optimize else "--null-connection")
    ...
    if conf.getPasswordHashes:
        try:
            conf.dumper.userSettings("数据库管理系统用户密码哈希", conf.dbmsHandler.getPasswordHashes(), "password hash", CONTENT_TYPE.PASSWORDS)
    ...
    if conf.getPrivileges:
        try:
            conf.dumper.userSettings("数据库管理系统用户权限", conf.dbmsHandler.getPrivileges(), "privilege", CONTENT_TYPE.PRIVILEGES)
        except SqlmapNoneDataException as ex:
            logger.critical(ex)
        except:
            raise
    ...
    if conf.getRoles:
        try:
            conf.dumper.userSettings("数据库管理系统用户角色", conf.dbmsHandler.getRoles(), "role", CONTENT_TYPE.ROLES)
        except SqlmapNoneDataException as ex:
            logger.critical(ex)
        except:
            raise
