...
def tableExists(tableFile, regex=None):
    if kb.choices.tableExists is None and not any(_ for _ in kb.injection.data if _ not in (PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED)) and not conf.direct:
        warnMsg = "不建议使用 '%s' 和/或 '%s' " % (PAYLOAD.SQLINJECTION[PAYLOAD.TECHNIQUE.TIME], PAYLOAD.SQLINJECTION[PAYLOAD.TECHNIQUE.STACKED])
        warnMsg += "用于公共表存在性检查"
    ...
    if result:
        errMsg = "由于检测到无效结果，无法使用表存在性检查 "
        errMsg += "(最有可能是由于使用过的注射器无法 "
        errMsg += "区分错误结果)"
    ...
    message = "您要使用哪个通用表（单词列表）文件？\n"
    message += "[1] 默认值 '%s' (按Enter键)\n" % tableFile
    message += "[2] 风俗"
    choice = readInput(message, default='1')

    if choice == '2':
        message = "自定义公共表文件的位置是什么？\n"
        tableFile = readInput(message) or tableFile

    infoMsg = "使用中的项执行表存在 '%s'" % tableFile
    logger.info(infoMsg)
        ...
        def tableExistsThread():
            ...
            while kb.threadContinue:
                ...
                if conf.verbose in (1, 2):
                    status = '%d/%d 项目 (%d%%)' % (threadData.shared.count, threadData.shared.limit, round(100.0 * threadData.shared.count / threadData.shared.limit))
                    dataToStdout("\r[%s] [信息] 尝试 %s" % (time.strftime("%X"), status), True)
...
def columnExists(columnFile, regex=None):
    ...
    if result:
        errMsg = "由于检测到无效结果，无法使用列存在性检查 "
        errMsg += "(最有可能是由于所用注射无法 "
        errMsg += "区分错误结果)"
        raise SqlmapDataException(errMsg)

    message = "您要使用哪个公共列（单词列表）文件？\n"
    message += "[1] 默认值 '%s' (按Enter键)\n" % columnFile
    message += "[2] 风俗"
    choice = readInput(message, default='1')

    if choice == '2':
        message = "自定义公共列文件的位置是什么？\n"
        columnFile = readInput(message) or columnFile

    infoMsg = "使用中的项检查列是否存在 '%s'" % columnFile
    logger.info(infoMsg)
...
def fileExists(pathFile):
    ...
    if choice == '2':
        message = "自定义通用文件文件位置是什么？\n"
        pathFile = readInput(message) or pathFile

    infoMsg = "使用中的项检查文件是否存在 '%s'" % pathFile
    logger.info(infoMsg)
