...
def task_delete(taskid):
    ...
    if taskid in DataStore.tasks:
        ...
        logger.debug("(%s) 已删除任务" % taskid)
        return jsonize({"success": True})
    else:
        response.status = 404
        logger.warning("[%s] 提供给task_delete()的任务ID不存在" % taskid)
...
def task_list(token=None):
    ...
    logger.debug("(%s) 列出的任务池 (%s)" % (token, "admin" if is_admin(token) else request.remote_addr))
...
def task_flush(token=None):
    ...
    logger.debug("(%s) 刷新的任务池 (%s)" % (token, "admin" if is_admin(token) else request.remote_addr))
    return jsonize({"success": True})
...
def option_get(taskid):
    ...
    """
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给option_get()的任务ID无效)" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    for option in options:
        if option in DataStore.tasks[taskid].options:
            ...
            logger.debug("(%s) 未知选项的请求值 '%s'" % (taskid, option))
            return jsonize({"success": False, "message": "Unknown option '%s'" % option})

    logger.debug("(%s) 已检索选项的值 '%s'" % (taskid, ','.join(options)))
...
def option_set(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给option_set()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    if request.json is None:
        logger.warning("[%s] 提供给option_set()的JSON选项无效)" % taskid)
...
def scan_start(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给scan_start()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    if request.json is None:
        logger.warning("[%s] 提供给scan_start()的JSON选项无效" % taskid)
        return jsonize({"success": False, "message": "Invalid JSON options"})

    for key in request.json:
        if key in RESTAPI_UNSUPPORTED_OPTIONS:
            logger.warning("[%s] 提供给scan_start()的选项 '%s' 不受支持" % (taskid, key))
...
def scan_stop(taskid):
    ...
    if (taskid not in DataStore.tasks or DataStore.tasks[taskid].engine_process() is None or DataStore.tasks[taskid].engine_has_terminated()):
        logger.warning("[%s] 提供给scan_stop()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def scan_kill(taskid):
    ...
    if (taskid not in DataStore.tasks or DataStore.tasks[taskid].engine_process() is None or DataStore.tasks[taskid].engine_has_terminated()):
        logger.warning("[%s] 提供给scan_kill()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def scan_status(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给scan_status（）的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    logger.debug("(%s) 检索到的扫描状态" % taskid)
    return jsonize({
        "success": True,
        "status": status,
...
def scan_data(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给scan_data()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    logger.debug("(%s)检索到的扫描数据和错误消息" % taskid)
...
def scan_log_limited(taskid, start, end):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s]提供给scan_log_limited()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    if not start.isdigit() or not end.isdigit() or int(end) < int(start):
        logger.warning("[%s] 提供给scan_log_limited()的起始值或结束值无效" % taskid)
    ...
    logger.debug("(%s) 检索到的扫描日志消息子集" % taskid)
...
def scan_log(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给scan_log()的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    logger.debug("(%s) 检索到的扫描日志消息" % taskid)
    return jsonize({"success": True, "log": json_log_messages})
...
def download(taskid, target, filename):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给download（）的任务ID无效" % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    if not path.startswith(paths.SQLMAP_OUTPUT_PATH):
        logger.warning("[%s] 禁止的路径 (%s)" % (taskid, target))
        return jsonize({"success": False, "message": "Forbidden path"})

    if os.path.isfile(path):
        logger.debug("(%s) 已检索文件的内容 %s" % (taskid, target))
...
def version(token=None):
    ...
    logger.debug("获取的版本 (%s)" % ("admin" if is_admin(token) else request.remote_addr))
...
def server(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, adapter=RESTAPI_DEFAULT_ADAPTER, username=None, password=None):
    ...
    logger.info("在上运行REST-JSON API服务器 '%s:%d'.." % (host, port))
    logger.info("管理员（机密）令牌: %s" % DataStore.admin_token)
    logger.debug("IPC数据库: '%s'" % Database.filepath)
...
def client(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, username=None, password=None):
    ...
    logger.info("将REST-JSON API客户端启动到 '%s'..." % addr)
    ...
    try:
        ...
        if not isinstance(ex, _urllib.error.HTTPError) or ex.code == _http_client.UNAUTHORIZED:
            errMsg = "连接到时出现问题 "
            errMsg += "REST-JSON API服务器位于'%s' " % addr
    ...
    while True:
        ...
        if command in ("data", "log", "status", "stop", "kill"):
            if not taskid:
                logger.error("没有正在使用的任务ID")
                continue
            raw = _client("%s/scan/%s/%s" % (addr, taskid, command))
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("无法执行命令 %s" % command)
            dataToStdout("%s\n" % raw)
            ...
            try:
                ...
                logger.error("分析参数时出错 ('%s')" % getSafeExString(ex))
                taskid = None
                continue
            ...
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("无法创建新任务 ('%s')" % res.get("message", ""))
                continue
            taskid = res["taskid"]
            ...
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("无法启动扫描 ('%s')" % res.get("message", ""))
                continue
            logger.info("扫描已启动")
            ...
            msg = "help           显示此帮助消息\n"
            msg += "new ARGS       使用提供的参数启动新的扫描任务 (e.g. 'new -u \"http://testphp.vulnweb.com/artists.php?artist=1\"')\n"
            msg += "use TASKID     将当前上下文切换到其他任务 (e.g. 'use c04d8c5c7582efb4')\n"
