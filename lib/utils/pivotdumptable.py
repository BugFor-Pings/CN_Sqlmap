...
def pivotDumpTable(table, colList, count=None, blind=True, alias=None):
    ...
    if conf.pivotColumn:
        for _ in colList:
            if re.search(r"(.+\.)?%s" % re.escape(conf.pivotColumn), _, re.I):
                infoMsg = "使用列 '%s' 作为轴心 " % conf.pivotColumn
                infoMsg += "用于检索行数据"
                logger.info(infoMsg)
        ...
        if not validPivotValue:
            warnMsg = "列 '%s' 不 " % conf.pivotColumn
            warnMsg += "在表中找到 '%s'" % table
            logger.warning(warnMsg)

    if not validPivotValue:
        for column in colList:
            infoMsg = "获取不同的数量 "
            infoMsg += "列的值 '%s'" % column.replace(("%s." % alias) if alias else "", "")
            logger.info(infoMsg)
            ...
            if isNumPosStrValue(value):
                ...
                if value == count:
                    infoMsg = "使用列 '%s' 作为轴心 " % column.replace(("%s." % alias) if alias else "", "")
                    infoMsg += "用于检索行数据"
                    logger.info(infoMsg)
    ...
    try:
            ...
            for column in colList:
                ...
                if conf.limitStart or conf.limitStop:
                    if conf.limitStart and (i + 1) < conf.limitStart:
                        warnMsg = "跳过第一个 %d 枢轴 " % conf.limitStart
                        warnMsg += "点值"
        ...
        errMsg = "检测到连接异常 ('%s'). sqlmap " % getSafeExString(ex)
        errMsg += "将显示部分输出"
