        ...
        def crawlThread():
            ...
            while kb.threadContinue:
                ...
                try:
                    ...
                    errMsg = "检测到连接异常 ('%s'). 跳过 " % getSafeExString(ex)
                    ...
                    errMsg = "检测到无效的URL ('%s'). 跳过 " % getSafeExString(ex)
                    errMsg += "URL '%s'" % current
                    logger.critical(errMsg)
        ...
        if kb.checkSitemap:
            ...
            try:
                ...
                if "page not found" in getSafeExString(ex):
                    found = False
                    logger.warning("'sitemap.xml' 找不到")
            except:
                pass
            finally:
                if found:
                    ...
                    logger.info("%s 找到链接" % ("no" if not items else len(items)))

        if not conf.bulkFile:
            infoMsg = "启动目标URL的爬网程序 '%s'" % target
            logger.info(infoMsg)
...
def storeResultsToFile(results):
    ...
    if kb.storeCrawlingChoice:
        ...
        infoMsg = "将爬网结果写入临时文件 '%s' " % filename
        logger.info(infoMsg)
