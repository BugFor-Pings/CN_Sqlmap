    ...
    def _get_cursor(self):
        ...
        if threadData.hashDBCursor is None:
            try:
                ...
                errMsg = "打开会话时出错 "
                errMsg += "文件 '%s' ('%s')" % (self.filepath, getSafeExString(ex))
    ...
    def retrieve(self, key, unserialize=False):
                ...
                for _ in xrange(HASHDB_RETRIEVE_RETRIES):
                    try:
                        ...
                        if any(_ in getSafeExString(ex) for _ in ("locked", "no such table")):
                            warnMsg = "访问会话文件时出现问题 '%s' ('%s')" % (self.filepath, getSafeExString(ex))
                            singleTimeWarnMessage(warnMsg)
                        elif "Could not decode" in getSafeExString(ex):
                            break
                        else:
                            errMsg = "访问会话文件时出错 '%s' ('%s'). " % (self.filepath, getSafeExString(ex))
                            errMsg += "如果问题仍然存在，请使用重新运行 '--flush-session'"
                            raise SqlmapConnectionException(errMsg)
                    else:
                        break
        ...
        if retVal and unserialize:
            try:
                ...
                retVal = None
                warnMsg = "取消序列化会话密钥的值时出错 '%s'. " % key
                warnMsg += "如果问题仍然存在，请使用重新运行 '--flush-session'"
