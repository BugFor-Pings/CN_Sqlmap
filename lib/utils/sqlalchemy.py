            ...
            try:
                if not self.port and self.db:
                    if not os.path.exists(self.db):
                        raise SqlmapFilePathException("提供的数据库文件 '%s' 不存在" % self.db)
                ...
                if "_get_server_version_info" in traceback.format_exc():
                    try:
                        ...
                        if int(pymssql.__version__[0]) < 2:
                            raise SqlmapConnectionException("SQLAlchemy连接问题（pymssql'%s') 的过时版本导致问题)" % pymssql.__version__)
                    except ImportError:
                        pass
                elif "invalid literal for int() with base 10: '0b" in traceback.format_exc():
                    raise SqlmapConnectionException("SQLAlchemy连接问题 ('https://bitbucket.org/zzzeek/sqlalchemy/issues/3975')")
                else:
                    pass
            except SqlmapFilePathException:
                raise
            except Exception as ex:
                raise SqlmapConnectionException("SQLAlchemy连接问题 ('%s')" % getSafeExString(ex))

            self.printConnected()
        else:
            raise SqlmapMissingDependence("SQLAlchemy不可用 (例子 'pip%s install SQLAlchemy')" % ('3' if six.PY3 else ""))
