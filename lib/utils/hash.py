...
def storeHashesToFile(attack_dict):
    ...
    if kb.choices.storeHashes is None:
        message = "是否要将哈希存储到临时文件中 "
        message += "最终用其他工具进行进一步加工 [y/N] "
...
def attackDumpedTable():
    if kb.data.dumpedTable:
        ...
        if binary_fields:
            ...
            warnMsg = "检测到潜在的二进制字段 ('%s'). 如果你有任何问题 " % _
            warnMsg += "建议重新运行表转储 '--fresh-queries --binary-fields=\"%s\"'" % _
        ...
        if attack_dict:
            infoMsg = "列中识别出可能的密码哈希 %s " % ("s" if len(col_passwords) > 1 else "")
            infoMsg += "'%s'" % ", ".join(col for col in col_passwords)
            logger.info(infoMsg)

            storeHashesToFile(attack_dict)

            message = "你想通过基于字典的攻击破解它们吗？ %s" % ("[y/N/q]" if conf.multipleTargets else "[Y/n/q]")
    ...
    try:
        for word in wordlist:
            ...
            try:
                ...
                if current in hashes:
                    for item in attack_info[:]:
                        ...
                        if hash_ == current:
                            ...
                            infoMsg = "\r[%s] [信息] 破解的密码 '%s'" % (time.strftime("%X"), word)
                ...
                warnMsg = "哈希项时出现问题：%s ('%s'). " % (repr(word), getSafeExString(ex))
                warnMsg += "请通过电子邮件向 '%s'" % DEV_EMAIL_ADDRESS
        ...
        for word in wordlist:
            ...
            try:
                ...
                if hash_ == current:
                    ...
                    if not api:
                        dataToStdout("\r[%s] [信息] %s" % (time.strftime("%X"), status))
...
def dictionaryAttack(attack_dict):
    ...
    for hash_regex in hash_regexes:
                    ...
                    try:
                        ...
                        if item and hash_ not in keys:
                            ...
                            if not resumed:
                                ...
                                infoMsg = "正在恢复哈希 '%s' 的密码 '%s'" % (resumed, hash_)
                                if user and not user.startswith(DUMMY_USER_PREFIX):
                                    infoMsg += " 对于用户 '%s'" % user
                                logger.info(infoMsg)
        ...
        if not kb.wordlists:
            ...
            message = "是否要使用通用密码后缀？（慢！） [y/N] "

            if readInput(message, default='N', boolean=True):
                suffix_list += COMMON_PASSWORD_SUFFIXES

        infoMsg = "启动基于字典的破解 (%s)" % __functions__[hash_regex].__name__
        logger.info(infoMsg)
                ...
                try:
                    if _multiprocessing:
                        if _multiprocessing.cpu_count() > 1:
                            infoMsg = "正在启动 %d 个进程 " % _multiprocessing.cpu_count()
                        ...
                        warnMsg = "目前正在进行多处理哈希破解 "
                        warnMsg += "%s 在这个平台上" % ("不受支持" if not conf.disableMulti else "丧失能力的")
                    ...
                    warnMsg = "用户在基于字典的攻击阶段中止（按下Ctrl+C）"
                    logger.warning(warnMsg)
