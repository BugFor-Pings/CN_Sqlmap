...
def _oneShotUnionUse(expression, unpack=True, limited=False):
    ...
    if retVal is None:
        ...
        if retVal is not None:
            ...
            if trimmed:
                warnMsg = "检测到可能的服务器调整输出 "
                warnMsg += "(可能是由于其长度和/或内容): "
                warnMsg += safecharencode(trimmed)
                logger.warning(warnMsg)

            elif re.search(r"ORDER BY [^ ]+\Z", expression):
                debugMsg = "在没有ORDER BY子句的情况下重试失败的SQL查询"
                ...
                debugMsg = "关闭'国家角色'铸造"  # NOTE: in some cases there are "known" incompatibilities between original columns and NCHAR (e.g. http://testphp.vulnweb.com/artists.php?artist=1)
                singleTimeDebugMessage(debugMsg)
...
def unionUse(expression, unpack=True, dump=False):
    ...
    if expressionFieldsList and len(expressionFieldsList) > 1 and "ORDER BY" in expression.upper():
        ...
        debugMsg = "从语句中剥离ORDER BY子句，因为 "
        debugMsg += "它不适合UNION查询SQL注入"
        singleTimeDebugMessage(debugMsg)
        ...
        if limitCond:
            ...
            if isNumPosStrValue(count) and int(count) > 1:
                ...
                try:
                    ...
                    errMsg = "边界限制 (%d,%d) 太大.请重新运行 " % (startLimit, stopLimit)
                    errMsg += "带开关的 '--fresh-queries'"
                ...
                if stopLimit > TURN_OFF_RESUME_INFO_LIMIT:
                    ...
                    debugMsg = "抑制大量行可能的恢复控制台信息, "
                    debugMsg += "因为这可能需要太长时间"
                    logger.debug(debugMsg)
                        ...
                        while kb.threadContinue:
                            ...
                            if output:
                                ...
                                if conf.verbose == 1 and not (threadData.resumed and kb.suppressResumeInfo) and not threadData.shared.showEta and not kb.bruteMode:
                                    ...
                                    status = "[%s] [信息] %s: %s" % (time.strftime("%X"), "resumed" if threadData.resumed else "retrieved", _ if kb.safeCharEncode else safecharencode(_))
    ...
    if not kb.bruteMode:
        debugMsg = "已执行 %d 查询%s 在 %.2f 秒" % (kb.counters[PAYLOAD.TECHNIQUE.UNION], 'y' if kb.counters[PAYLOAD.TECHNIQUE.UNION] == 1 else "ies", duration)
        logger.debug(debugMsg)
