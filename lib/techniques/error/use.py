    ...
    for field in expressionFieldsList:
        ...
        if not any((suppressOutput, kb.bruteMode)):
            if kb.fileReadMode and output and output.strip():
                ...
                status = "[%s] [信息] %s: '%s'" % (time.strftime("%X"), "resumed" if threadData.resumed else "retrieved", output if kb.safeCharEncode else safecharencode(output))
...
def errorUse(expression, dump=False):
                ...
                if kb.dumpTable and (len(expressionFieldsList) < (stopLimit - startLimit) > CHECK_ZERO_COLUMNS_THRESHOLD):
                    for field in expressionFieldsList:
                        if _oneShotErrorUse("SELECT COUNT(%s) FROM %s" % (field, kb.dumpTable)) == '0':
                            ...
                            debugMsg = "表 '%s'的列 '%s' 将不会 " % (field, kb.dumpTable)
                            debugMsg += "转储为空"
                            logger.debug(debugMsg)

                if stopLimit > TURN_OFF_RESUME_INFO_LIMIT:
                    kb.suppressResumeInfo = True
                    debugMsg = "由于行数较多，因此抑制可能的恢复控制台信息 "
                    debugMsg += "可能需要太长时间"
                    logger.debug(debugMsg)
    ...
    if not kb.bruteMode:
        debugMsg = "已执行 %d 查询%s 在 %.2f 秒" % (kb.counters[getTechnique()], 'y' if kb.counters[getTechnique()] == 1 else "ies", duration)
        logger.debug(debugMsg)
