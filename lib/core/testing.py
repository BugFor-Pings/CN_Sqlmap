...
def vulnTest():
    ...
    if not vulnserver._alive:
        logger.error("vulnserver实例化中出现问题（地址：'http://%s:%s')" % (address, port))
        return False
    else:
        logger.info("vulnserver正在运行 'http://%s:%s'..." % (address, port))
    ...
    clearConsoleLine()
    if retVal:
        logger.info("vuln测试最终结果：通过")
    else:
        logger.error("vuln测试最终结果：失败")
        ...
        for filename in files:
            if os.path.splitext(filename)[1].lower() == ".py" and filename not in ("__init__.py", "gui.py"):
                ...
                try:
                    ...
                    retVal = False
                    dataToStdout("\r")
                    errMsg = "导入模块'%s' (%s):\n%s 时烟雾测试失败" % (path, os.path.join(root, filename), ex)
                    logger.error(errMsg)
                else:
                ...
                dataToStdout("\r[%s] [信息] 完成: %s" % (time.strftime("%X"), status))
