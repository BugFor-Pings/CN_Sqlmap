...
class Format(object):
    ...
    def getOs(target, info):
        ...
        if "technology" in info:
            if conf.api:
                infoApi["web 应用技术"] = Format.humanize(info["technology"], ", ")
            else:
                infoStr += "\nweb 应用技术: %s" % Format.humanize(info["technology"], ", ")
...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "sqlmap以前的指纹后端DBMS为"
            msg += "%s. 然而，现在它已经被指纹 " % kb.dbms
            msg += "为 %s. " % dbms
            msg += "请请指定应该是哪个DBMS"
            msg += "应该是正确的[%s (default)/%s] " % (kb.dbms, dbms)
    ...
    def setOs(os):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "sqlmap以前的指纹后端DBMS操作系统 "
            msg += " %s. 然而，现在它已经 " % kb.os
            msg += "被指纹 %s. " % os
            msg += "请指定正确的操作系统 "
            msg += " [%s (默认)/%s] " % (kb.os, os)
    ...
    def isDbms(dbms):
        if not kb.get("testMode") and all((Backend.getDbms(), Backend.getIdentifiedDbms())) and Backend.getDbms() != Backend.getIdentifiedDbms():
            singleTimeWarnMessage("已识别('%s') 和指纹（'%s'）的DBMS不同，如果在枚举阶段遇到问题，请使用'--flush-session'重新运行" % (Backend.getIdentifiedDbms(), Backend.getDbms()))
            ...
            if condition:
                ...
                if parameter in (conf.base64Parameter or []):
                    try:
                        ...
                        errMsg = " 参数'%s'不包含有效的" % parameter
                        errMsg += "Base64编码值('%s')" % value
                ...
                if not conf.multipleTargets and not (conf.csrfToken and re.search(conf.csrfToken, parameter, re.I)):
                    ...
                    if (_.endswith("'") and _.count("'") == 1 or re.search(r'\A9{3,}', _) or re.search(r'\A-\d+\Z', _) or re.search(DUMMY_USER_INJECTION, _)) and not parameter.upper().startswith(GOOGLE_ANALYTICS_COOKIE_PREFIX):
                        warnMsg = "似乎您提供了受污染的参数值 "
                        warnMsg += "('%s') 最有可能是" % element
                        warnMsg += "来自手动SQL注入测试的剩余字符/语句(s). "
                        warnMsg += "请始终只使用有效的参数值"
                        warnMsg += "以便sqlmap能够正常运行"
                        logger.warning(warnMsg)

                        message = "确实要继续吗（sqlmap可能有问题）？ [y/N] "

                        if not readInput(message, default='N', boolean=True):
                            raise SqlmapSilentQuitException
                    elif not _:
                        warnMsg = "为参数'%s' 提供的值为空. " % parameter
                        warnMsg += "请始终只使用有效的参数值"
                        warnMsg += "以便sqlmap能够正常运行"
                        logger.warning(warnMsg)
                    ...
                    for regex in (r"\A((?:<[^>]+>)+\w+)((?:<[^>]+>)+)\Z", r"\A([^\w]+.*\w+)([^\w]+)\Z"):
                        ...
                        if match:
                            try:
                                ...
                                if candidates:
                                    message = "似乎为 %s 参数 '%s'提供了值 " % ("%s " % place if place != parameter else "", parameter)
                                    message += "是JSON可反序列化的。你想在里面注射吗？ [y/N] "
                            ...
                            message = "似乎为 %s 参数 '%s'提供了值 " % ("%s " % place if place != parameter else "", parameter)
                            message += "有边界，你想在里面注射吗？('%s') [y/N] " % getUnicode(_)
                ...
                for encoding in ("hex", "base64"):
                    try:
                        ...
                        if len(decoded) > MIN_ENCODED_LEN_CHECK and all(_ in getBytes(string.printable) for _ in decoded):
                            warnMsg = "提供的参数 '%s' " % parameter
                            warnMsg += "似乎是'%s' 编码" % encoding
                            logger.warning(warnMsg)
                            break
                    except:
                        pass
...
def getManualDirectories():
    ...
    if conf.webRoot:
        ...
        infoMsg = "使用 '%s' 作为web服务器root" % conf.webRoot
        logger.info(infoMsg)
    elif directories:
        infoMsg = "检索到web服务器root: '%s'" % directories
        logger.info(infoMsg)
    else:
        warnMsg = "无法自动检索web服务器 "
        warnMsg += "文档根目录"
        logger.warning(warnMsg)
...
def dataToTrafficFile(data):
    ...
    try:
        ...
        errMsg = "试图写入"
        errMsg += " 流量文件'%s' ('%s')出错" % (conf.trafficFile, getSafeExString(ex))
...
def dataToDumpFile(dumpFile, data):
    try:
        ...
        if "No space left" in getUnicode(ex):
            ...
            errMsg = "将转储数据写入文件时出错('%s')" % getUnicode(ex)
            logger.error(errMsg)
        ...
        while True:
            ...
            try:
                ...
                if filename != _:
                    ...
                    errMsg = "无法写入"
                    errMsg += " 输出文件('%s')" % getSafeExString(ex)
                    raise SqlmapGenericException(errMsg)
            except IOError as ex:
                errMsg = "尝试写入输出文件 "
                errMsg += "时出错('%s')" % getSafeExString(ex)
                raise SqlmapGenericException(errMsg)
            else:
                break
...
def checkFile(filename, raiseOnError=True):
    ...
    if not valid and raiseOnError:
        raise SqlmapSystemException("无法读取文件 '%s'" % filename)
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    if remote:
                        warnMsg = " 不支持通过网络直接连接"
                        warnMsg += "%s DBMS " % dbmsName
                        logger.warning(warnMsg)

                        conf.hostname = "localhost"
                        conf.port = 0
                elif not remote:
                    errMsg = "缺少远程连接详细信息 (e.g. "
                    errMsg += "'mysql://USER:PASSWORD@DBMS_IP:DBMS_PORT/DATABASE_NAME' "
                    errMsg += "或 'access://DATABASE_FILEPATH')"
                    raise SqlmapSyntaxException(errMsg)
                ...
                if dbmsName in (DBMS.MSSQL, DBMS.SYBASE):
                    ...
                    if not hasattr(pymssql, "__version__") or pymssql.__version__ < "1.0.2":
                        errMsg = "'%s' 第三方库必须是 " % data[1]
                        errMsg += "版本 >= 1.0.2 才能正常工作. "
                        errMsg += "下载 '%s'" % data[2]
                ...
                if _sqlalchemy and data[3] and any(_ in _sqlalchemy.dialects.__all__ for _ in (data[3], data[3].split('+')[0])):
                    ...
                    errMsg = "sqlmap需要 '%s' 第三方库 " % data[1]
                    errMsg += "以便直接连接到DBMS "
                    errMsg += "'%s'. 您可以从 '%s'" % (dbmsName, data[2])
                    errMsg += ". 另一种方法是使用包'python-sqlalchemy' "
                    errMsg += "已安装方言'%s'支持" % data[3]
                    raise SqlmapMissingDependence(errMsg)
...
def parseTargetUrl():
    ...
    try:
        ...
        errMsg = "提供了无效的URL'%s' ('%s'). " % (conf.url, getSafeExString(ex))
        errMsg += "请确保您没有任何剩余字符 (e.g. '[' or ']') "
        errMsg += "在主机名部分"
        raise SqlmapGenericException(errMsg)
    ...
    if (intersect(REFERER_ALIASES, conf.testParameter, True) or conf.level >= 3) and not any(_[0].upper() == HTTP_HEADER.REFERER.upper() for _ in conf.httpHeaders):
        debugMsg = "将HTTP Refer标头设置为目标URL"
        logger.debug(debugMsg)
...
def getLocalIP():
    ...
    try:
        ...
        debugMsg = "打开套接字时出错 "
        debugMsg += "朝向的连接 '%s'" % conf.hostname
        logger.debug(debugMsg)
    ...
    if isinstance(params, six.string_types):
        ...
        if retVal.count("%s", start, end) == len(params):
            ...
            while True:
                ...
                if match:
                    if count >= len(params):
                        warnMsg = "字符串格式化期间参数数量错误. "
                        warnMsg += "请通过电子邮件内容报告 \"%r | %r | %r\" to '%s'" % (format_, params, retVal, DEV_EMAIL_ADDRESS)
...
def parseXmlFile(xmlFile, handler):
    ...
    try:
        ...
        errMsg = "文件 "
        errMsg += "'%s' ('%s')似乎有问题.请确保您没有对 " % (xmlFile, getSafeExString(ex))
        errMsg += "其进行任何更改"
...
def getSQLSnippet(dbms, sfile, **variables):
    ...
    if variables:
        errMsg = "SQL文件%s 中未解析的变量'%s' '%s'" % ("s" if len(variables) > 1 else "", ", ".join(variables), sfile)
        logger.error(errMsg)
        ...
        with kb.locks.cache:
            if filename not in kb.cache.content:
                ...
                try:
                    ...
                    errMsg = "尝试读取文件内容 "
                    errMsg += "时出错 '%s' ('%s')" % (filename, getSafeExString(ex))
                    raise SqlmapSystemException(errMsg)
...
def getFileItems(filename, commentPrefix='#', unicoded=True, lowercase=False, unique=False):
    ...
    try:
        ...
        errMsg = "尝试时出错 "
        errMsg += "读取文件的内容 '%s' ('%s')" % (filename, getSafeExString(ex))
        raise SqlmapSystemException(errMsg)
...
def wasLastResponseDelayed():
    ...
    if deviation and not conf.direct and not conf.disableStats:
        if len(kb.responseTimes[kb.responseTimeMode]) < MIN_TIME_RESPONSES:
            warnMsg = "在响应时间少于 "
            warnMsg += "%d的模型上使用基于时间的标准差方法" % MIN_TIME_RESPONSES
            logger.warning(warnMsg)
        ...
        if not kb.testMode and retVal:
            if kb.adjustTimeDelay is None:
                msg = "是否希望sqlmap尝试优化值 "
                msg += "对于DBMS延迟响应 (选项 '--time-sec')? [Y/n] "
...
def adjustTimeDelay(lastQueryDuration, lowerStdLimit):
    ...
    if all((_ == candidate for _ in kb.delayCandidates)) and candidate < conf.timeSec:
        if lastQueryDuration / (1.0 * conf.timeSec / candidate) > MIN_VALID_DELAYED_RESPONSE:  # Note: to prevent problems with fast responses for heavy-queries like RANDOMBLOB
            ...
            infoMsg = "由于响应时间良好，将时间 "
            infoMsg += "延迟调整为%d 秒%s " % (conf.timeSec, 's' if conf.timeSec > 1 else '')
            logger.info(infoMsg)
...
def urlencode(value, safe="%&=-_", convall=False, limit=False, spaceplus=False):
    ...
    if value:
        ...
        if Backend.isDbms(DBMS.MSSQL) and not kb.tamperFunctions and any(ord(_) > 255 for _ in value):
            warnMsg = "如果您遇到以下问题 "
            warnMsg += "non-ASCII标识符名称 "
            warnMsg += "建议您重新运行 '--tamper=charunicodeencode'"
...
def runningAsAdmin():
    ...
    if PLATFORM in ("posix", "mac"):
        ...
        errMsg = "sqlmap无法检查您是否以管理员帐户 "
        errMsg += "在该平台上运行它. "
        errMsg += "sqlmap将假设您是管理员 "
        errMsg += "这是所请求的接管攻击"
        errMsg += "正常工作所必需的"
        logger.error(errMsg)
...
def setOptimize():
    ...
    if not conf.nullConnection:
        debugMsg = "关闭开关 '-o' 间接使用的开关'-null-connection'"
        logger.debug(debugMsg)
...
def saveConfig(conf, filename):
    ...
    with openFile(filename, "wb") as f:
        try:
            ...
            errMsg = "尝试时出错 "
            errMsg += "写入配置文件 '%s' ('%s')" % (filename, getSafeExString(ex))
            raise SqlmapSystemException(errMsg)
...
def initTechnique(technique=None):
    ...
    try:
        ...
        if data:
            ...
            warnMsg = "没有可用于技术的注入数据 "
            warnMsg += "'%s'" % enumValueToNameLookup(PAYLOAD.TECHNIQUE, technique)
            logger.warning(warnMsg)

    except SqlmapDataException:
        errMsg = "旧会话文件中缺少数据. "
        errMsg += "请使用'--flush-session'进行交易"
        errMsg += "出现此错误"
...
def showHttpErrorCodes():
    ...
    if kb.httpErrorCodes:
        ...
        if any((str(_).startswith('4') or str(_).startswith('5')) and _ != _http_client.INTERNAL_SERVER_ERROR and _ != kb.originalCode for _ in kb.httpErrorCodes):
            msg = "4xx和/或5xx HTTP错误代码太多 "
            msg += "可能意味着涉及某种保护 (例如. WAF)"
            logger.debug(msg)
...
def openFile(filename, mode='r', encoding=UNICODE_ENCODING, errors="reversible", buffering=1):  # "buffering=1" means line buffered (Reference: http://stackoverflow.com/a/3168436)
    ...
    if filename == STDIN_PIPE_DASH:
        ...
        try:
            ...
            errMsg = "文件名出现文件打开错误 '%s'. " % filename
            errMsg += "请检查文件的 %s 权限 " % ("write" if mode and ('w' in mode or 'a' in mode or '+' in mode) else "read")
            errMsg += "并且该文件未被其他进程锁定"
            raise SqlmapSystemException(errMsg)
...
def unhandledExceptionMessage():
    ...
    errMsg = "%s 中发生未处理的异常. 建议您重试 " % VERSION_STRING
    errMsg += "使用官方GitHub的最新开发版本运行"
    errMsg += "存储库位于 '%s'. 如果异常仍然存在，请打开新问题 " % GIT_PAGE
    errMsg += "at '%s' " % ISSUES_PAGE
    errMsg += "带有以下文本和再现错误所需的任何其他信息"
    errMsg += "开发人员将尝试重现该错误. 开发人员将尝试复制该错误,并相应地修复它 "
    errMsg += "然后返回给您\n"
...
def fetchRandomAgent():
    ...
    if not kb.userAgents:
        debugMsg = "从加载随机HTTP用户代理标头"
        debugMsg += "文件 '%s'" % paths.USER_AGENTS
        logger.debug(debugMsg)

        try:
            kb.userAgents = getFileItems(paths.USER_AGENTS)
        except IOError:
            errMsg = "无法读取HTTP用户代理标头 "
            errMsg += "文件 '%s'" % paths.USER_AGENTS
            raise SqlmapSystemException(errMsg)
...
def createGithubIssue(errMsg, excMsg):
    ...
    if choice:
        ...
        try:
            ...
            if duplicate:
                warnMsg = "这个问题似乎已经被报道了"
                if closed:
                    warnMsg += " 并解决。请更新至最新版本"
                    warnMsg += "GitHub官方存储库的开发版本 '%s'" % GIT_PAGE
                logger.warning(warnMsg)
                return
        except:
            pass
        ...
        if issueUrl:
            infoMsg = "可以在地址找到创建的Github问题 '%s'" % issueUrl.group(0)
            logger.info(infoMsg)
            ...
            if regex != payload:
                ...
                if retVal != content:
                    ...
                    if re.search(r"(?i)FRAME[^>]+src=[^>]*%s" % REFLECTED_VALUE_MARKER, retVal):
                        warnMsg = "检测到包含受攻击的参数值的帧 "
                        warnMsg += "请确保单独测试这些，以防对此页面的攻击失败"
                        singleTimeWarnMessage(warnMsg)
    ...
    for mnemonic in (mnemonics or "").split(','):
        ...
        if pointer in (None, head):
            ...
            if not options:
                ...
                warnMsg = "检测到歧义（助记符 '%s' 可以解析为以下任一项: %s). " % (name, ", ".join("'%s'" % key for key in options))
                warnMsg += "解析为其中最短的 ('%s')" % found
                logger.warning(warnMsg)
...
def findPageForms(content, url, raise_=False, addToTargets=False):
    ...
    try:
        ...
        if re.search(r"(?i)<!DOCTYPE html|<html", content or "") and not re.search(r"(?i)\.js(\?|\Z)", url):
            dbgMsg = "给定URL('%s') 的HTML格式不正确，去过滤它" % url
            logger.debug(dbgMsg)
    ...
    for form in forms or []:
        try:
            ...
            errMsg = "处理页表单 "
            errMsg += " ('%s')时出现问题" % getSafeExString(ex)
            if raise_:
                raise SqlmapGenericException(errMsg)
            else:
                logger.debug(errMsg)
...
def checkSystemEncoding():
    ...
    if sys.getdefaultencoding() == "cp720":
        try:
            ...
            errMsg = "有一个已知的Python问题 (#1616979) 与"
            errMsg += "支持字符集'cp720'. 请访问 "
...
def evaluateCode(code, variables=None):
    ...
    try:
        ...
        errMsg = "评估提供的代码时出错 ('%s') " % getSafeExString(ex)
        raise SqlmapGenericException(errMsg)
    ...
    def _(value):
        ...
        if value and isinstance(value, six.string_types):
            ...
            if len(value) % 2 != 0:
                ...
                singleTimeWarnMessage("从预期的十六进制格式解码值 '%s' 时出现问题" % value)
            else:
    ...
    if not conf.loadCookies:
        ...
        try:
            if not cookieJar.filename:
                infoMsg = "c从 '%s'加载cookie" % conf.loadCookies
                logger.info(infoMsg)
            ...
            errMsg = "加载时出现问题 "
            errMsg += "cookies 文件 ('%s')" % re.sub(r"(cookies) file '[^']+'", r"\g<1>", getSafeExString(ex))
            raise SqlmapGenericException(errMsg)
        ...
        if returncode is not None:
            if not suppress_errors:
                if returncode == 0:
                    ...
                    dataToStdout(" 进程被信号终止 %d\n" % returncode)
                elif returncode > 0:
                    dataToStdout(" 意外退出并返回代码 %d\n" % returncode)
...
def parseRequestFile(reqFile, checkParams=True):
    ...
    def _parseWebScarabLog(content):
        ...
        for request in reqResList:
            ...
            if not method or not url:
                logger.debug("不是有效的WebScarab日志数据")
                continue

            if method.upper() == HTTPMETHOD.POST:
                warnMsg = " 来自WebScarab日志的POST请求不受支持"
                warnMsg += "因为它们的主体内容存储在单独的文件中. "
                warnMsg += "然而，您可以使用-r单独加载它们."
                logger.warning(warnMsg)
                continue
    ...
    if conf.scope:
        logger.info("对筛选目标使用正则表达式 '%s' " % conf.scope)
