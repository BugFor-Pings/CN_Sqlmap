...
class Format(object):
    ...
    def getDbms(versions=None):
        """
        格式化后端数据库管理系统（DBMS）的指纹值，并返回其
        以人类可读字符串格式化的值。

        @return: 基于指纹技术检测到的后端 DBMS。
        @rtype: C{str}
        """
        ...
        # 注意：防止出现丑陋的（例如）“后端 DBMS: MySQL Unknown”
    ...
    def getErrorParsedDBMSes():
        """
        解析知识库 htmlFp 列表，并返回其值
        以人类可读字符串格式化。

        @return: 基于错误信息解析的可能后端 DBMS 列表。
        @rtype: C{str}
        """
    ...
    def getOs(target, info):
        ...
        if "technology" in info:
            if conf.api:
                infoApi["web server 应用技术"] = Format.humanize(info["technology"], ", ")
            else:
                infoStr += "\nweb server 应用技术: %s" % Format.humanize(info["technology"], ", ")
...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # 小心一点，理论上这个条件应该总是为假
            ...
            msg = "sqlmap以前将后端DBMS指纹识别为 "
            msg += "%s。 然而现在它被指纹识别为 " % kb.dbms
            msg += "%s。 " % dbms
            msg += "请明确指定哪个DBMS是 "
            msg += "正确的 [%s (默认)/%s] " % (kb.dbms, dbms)
    ...
    def setOs(os):
        ...
        # 小心一点，理论上这个条件应该总是为假
            ...
            msg = "sqlmap以前将后端DBMS "
            msg += "操作系统指纹识别为 %s。 然而现在它 " % kb.os
            msg += "被指纹识别为 %s。 " % os
            msg += "请明确指定哪个操作系统是 "
            msg += "正确的 [%s (默认)/%s] " % (kb.os, os)
...
def paramToDict(place, parameters=None):
    """
    将参数拆分为名称和值，检查这些参数是否在可测试参数内，并以字典形式返回。
    """
            ...
            if condition:
                ...
                if parameter in (conf.base64Parameter or []):
                    try:
                        ...
                        errMsg = "参数 '%s' 不包含 " % parameter
                        errMsg += "有效的 Base64 编码值 ('%s')" % value
                ...
                if not conf.multipleTargets and not (conf.csrfToken and re.search(conf.csrfToken, parameter, re.I)):
                    ...
                    if (_.endswith("'") and _.count("'") == 1 or re.search(r'\A9{3,}', _) or re.search(r'\A-\d+\Z', _) or re.search(DUMMY_USER_INJECTION, _)) and not parameter.upper().startswith(GOOGLE_ANALYTICS_COOKIE_PREFIX):
                        warnMsg = "您提供的参数值似乎含有受污染的值 "
                        warnMsg += "('%s')，可能是手动 SQL 注入测试的剩余字符/语句。 " % element
                        warnMsg += "请始终使用有效的参数值 "
                        warnMsg += "以便 sqlmap 能够正常运行。"
                        logger.warning(warnMsg)

                        message = "您确定要继续吗 (sqlmap 可能会出现问题)? [y/N] "

                        if not readInput(message, default='N', boolean=True):
                            raise SqlmapSilentQuitException
                    elif not _:
                        warnMsg = "参数 '%s' 的提供值为空。 " % parameter
                        warnMsg += "请始终使用有效的参数值 "
                        warnMsg += "以便 sqlmap 能够正常运行。"
                        logger.warning(warnMsg)
                    ...
                    for regex in (r"\A((?:<[^>]+>)+\w+)((?:<[^>]+>)+)\Z", r"\A([^\w]+.*\w+)([^\w]+)\Z"):
                        ...
                        if match:
                            try:
                                ...
                                # 注意：对于包含自定义注入标记的情况（例如：https://github.com/sqlmapproject/sqlmap/issues/4137#issuecomment-2013783111） - p.s. 对结构不太关心（例如，注入到扁平数组值中）
                                ...
                                if candidates:
                                    message = "提供的值似乎是可 JSON 反序列化的。您想要在其中注入吗? [y/N] "
                            ...
                            message = "提供的值似乎有边界。您想要在其中注入吗? ('%s') [y/N] " % getUnicode(_)
...
def getManualDirectories():
    ...
    if conf.webRoot:
        ...
        infoMsg = "使用 '%s' 作为 web 服务器文档根目录" % conf.webRoot
        logger.info(infoMsg)
    elif directories:
        infoMsg = "获取到的 web 服务器文档根目录: '%s'" % directories
        logger.info(infoMsg)
    else:
        warnMsg = "无法自动获取 web 服务器的 "
        warnMsg += "文档根目录"
        logger.warning(warnMsg)
        ...
        if choice == '2':
            message = "请输入以逗号分隔的绝对目录路径列表: "
            directories = readInput(message, default="").split(',')
        elif choice == '3':
            message = "目录列表文件的位置是什么？\n"
...
def dataToTrafficFile(data):
    ...
    try:
        ...
        errMsg = "在尝试写入流量文件 '%s' 时出现问题 ('%s')" % (conf.trafficFile, getSafeExString(ex))
...
def dataToOutFile(filename, data):
    ...
    if data:
        while True:
            ...
            try:
                with open(retVal, "w+b") as f:  # 必须保持为非编解码方式，因为数据是原始的 ASCII 编码数据
                ...
                errMsg = "在尝试写入时出现问题 "
                errMsg += "到输出文件 ('%s')" % getSafeExString(ex)
                raise SqlmapGenericException(errMsg)
            else:
                break
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    if remote:
                        warnMsg = "通过网络直接连接 "
                        warnMsg += "%s 数据库管理系统不被支持" % dbmsName
                        logger.warning(warnMsg)
                ...
                if dbmsName in (DBMS.MSSQL, DBMS.SYBASE):
                    ...
                    # 参考: http://itsiti.com/ora-28009-connection-sys-sysdba-sysoper
                ...
                if _sqlalchemy and data[3] and any(_ in _sqlalchemy.dialects.__all__ for _ in (data[3], data[3].split('+')[0])):
                    ...
                    errMsg = "sqlmap 需要 '%s' 第三方库 " % data[1]
                    errMsg += "以便直接连接到数据库管理系统 "
                    errMsg += "'%s'。你可以从 '%s' 下载" % (dbmsName, data[2])
                    errMsg += "。替代方案是安装支持 '%s' 方言的 'python-sqlalchemy' 包" % data[3]
                    raise SqlmapMissingDependence(errMsg)
...
def unhandledExceptionMessage():
    ...
    >>> all(_ in unhandledExceptionMessage() for _ in ("未处理的异常发生", "操作系统", "命令行"))
    True
    """

    errMsg = "在 %s 中发生了未处理的异常。建议您使用最新的开发版本重试 " % VERSION_STRING
    errMsg += "从官方 GitHub "
    errMsg += "如果异常持续存在请在仓库 '%s' 获取。 " % GIT_PAGE
    errMsg += "上提交一个新问题 '%s' " % ISSUES_PAGE
    errMsg += "并附上以下文本以及任何其他信息，以 "
    errMsg += "重现该错误。开发人员将尝试重现该错误，进行相应修复 "
    errMsg += "并及时回复您\n"
