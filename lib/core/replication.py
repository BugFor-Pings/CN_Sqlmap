...
class Replication(object):
    ...
    def __init__(self, dbpath):
        try:
            ...
            errMsg = "打开复制时出错 "
            errMsg += "文件 '%s' ('%s')" % (dbpath, getSafeExString(ex))
    ...
    class Table(object):
        ...
        def __init__(self, parent, name, columns=None, create=True, typeless=False):
            ...
            if create:
                try:
                    ...
                    errMsg = "初始化sqlite数据库时出现问题 ('%s')  " % getSafeExString(ex, UNICODE_ENCODING)
                    errMsg += "位于 '%s'" % self.parent.dbpath
        ...
        def execute(self, sql, parameters=None):
            try:
                ...
                errMsg = "访问位于 ('%s')的sqlite数据库时 " % getSafeExString(ex, UNICODE_ENCODING)
                errMsg += "出现问题 '%s'. 请确保" % self.parent.dbpath
                errMsg += "其他程序没有使用它"
                raise SqlmapGenericException(errMsg)
