...
def saveHistory(completion=None):
    try:
        ...
        try:
            ...
            warnMsg = "写入历史文件时出现问题 '%s' (%s)" % (historyPath, getSafeExString(ex))
...
def loadHistory(completion=None):
    ...
    if os.path.exists(historyPath):
        try:
            ...
            warnMsg = "加载历史文件时出现问题 '%s' (%s)" % (historyPath, getSafeExString(ex))
            logger.warning(warnMsg)
        except UnicodeError:
            if IS_WIN:
                warnMsg = "加载历史文件时出现问题 '%s'. " % historyPath
                warnMsg += "更多信息请访问 'https://github.com/pyreadline/pyreadline/issues/30'"
                logger.warning(warnMsg)
