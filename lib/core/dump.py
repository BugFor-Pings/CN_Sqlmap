...
class Dump(object):
    ...
    def currentDb(self, data):
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.PGSQL, DBMS.HSQLDB, DBMS.H2, DBMS.MONETDB, DBMS.VERTICA, DBMS.CRATEDB, DBMS.CACHE, DBMS.FRONTBASE):
            self.string("当前数据库 (等效于%s上的架构)" % Backend.getIdentifiedDbms(), data, content_type=CONTENT_TYPE.CURRENT_DB)
    ...
    def hostname(self, data):
        self.string("主机名", data, content_type=CONTENT_TYPE.HOSTNAME)

    def dba(self, data):
        self.string("当前用户是DBA", data, content_type=CONTENT_TYPE.IS_DBA)
    ...
    def dbs(self, dbs):
        ...
        try:
            open('gui_data/dbs.txt','w').close()
            with open('gui_data/dbs.txt', 'a+') as f:
                for file_db in dbs:
                    f.write(file_db+'\n')
            #     print(file_db)
            # print(dbs)
    ...
    def dbTables(self, dbTables):
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            for db, tables in dbTables.items():
                ...
                self._write("数据库: %s" % unsafeSQLIdentificatorNaming(db) if db and METADB_SUFFIX not in db else "<current>")
                #print(tables)
                try:
                    open('gui_data/tables.txt','w').close()
                    with open('gui_data/tables.txt','a+') as f1:
                        for file_table in tables:
                            f1.write(file_table+'\n')
                except:
                    pass
                if len(tables) == 1:
            ...
            for db, tables in tableColumns.items():
                ...
                for table, columns in tables.items():
                    ...
                    for column in colList:
                        try:
                            with open('gui_data/columns.txt','a+') as f2:
                                f2.write(column+'\n')
                        except:
                            pass
    ...
    def dbTableValues(self, tableValues):
        ...
        try:
            ...
            try:
                ...
                warnMsg = "当前无法使用常规转储目录. "
                warnMsg += "改用临时目录 '%s'" % tempDir
        ...
        data_values=tableValues[columns[0]]['values']
        try:
            open('gui_data/values.txt','w').close()
            for data_value in data_values:
                with open('gui_data/values.txt','a+') as f3:
                    f3.write(data_value+'\n')
                #print(data_va)
        except:
            pass
