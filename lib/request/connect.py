...
class Connect(object):
    ...
    def _retryProxy(**kwargs):
        ...
        if kb.testMode and kb.previousMethod == PAYLOAD.METHOD.TIME:
            ...
            warnMsg = "最有可能的是"
            warnMsg += "web服务器实例尚未从以前的基于时间的负载中恢复 "
            warnMsg += " 如果问题仍然存在，请等待几分钟 "
            warnMsg += "然后在选项'--technique'中不带标志'T'的情况下重新运行"
            warnMsg += "(例子 '--flush-session --technique=BEUS') 或尝试 "
            warnMsg += "降低选项的值 '--time-sec' (例子 '--time-sec=2')"
            ...
            if conf.tor:
                ...
                warnMsg += "开关'--Tor' "
                if IS_WIN:
                    warnMsg += "(例如 'https://www.torproject.org/download/')"
                else:
                    warnMsg += "(例如 'https://help.ubuntu.com/community/Tor')"
            else:
                warnMsg = "如果问题仍然存在，请检查提供的 "
                warnMsg += "目标URL是否可访问"
                ...
                if items:
                    warnMsg += ".如果是的话, "
                    warnMsg += "您可以尝试重新运行 "
                    warnMsg += " 和/或 ".join(items)

            singleTimeWarnMessage(warnMsg)

        elif conf.threads > 1:
            warnMsg = " 如果问题仍然存在"
            warnMsg += "请尝试减少使用的线程数 (选项 '--threads')"
            singleTimeWarnMessage(warnMsg)
    ...
    def getPage(**kwargs):
        ...
        if conf.liveCookies:
            with kb.locks.liveCookies:
                if not checkFile(conf.liveCookies, raiseOnError=False) or os.path.getsize(conf.liveCookies) == 0:
                    warnMsg = "[%s] [警告] 实时cookie文件'%s' 为空或不存在,正在等待超时 (%d 秒)" % (time.strftime("%X"), conf.liveCookies, LIVE_COOKIES_TIMEOUT)
                    ...
                    if not valid:
                        errMsg = "从文件加载cookie时出现问题 '%s'" % conf.liveCookies
        ...
        try:
            ...
            if headers.get(HTTP_HEADER.CONTENT_TYPE) == POST_HINT_CONTENT_TYPES[POST_HINT.MULTIPART]:
                warnMsg = "'%s'标头中缺少'boundary parameter' " % HTTP_HEADER.CONTENT_TYPE
                warnMsg += "将尝试重建"
                singleTimeWarnMessage(warnMsg)
            ...
            if webSocket:
                ...
                for function in kb.preprocessFunctions:
                    try:
                        ...
                        errMsg = "运行预处理 "
                        errMsg += "函数 '%s' ('%s')出错" % (function.__name__, getSafeExString(ex))
            ...
            if not refreshing:
                ...
                if not refresh:
                    ...
                    if refresh:
                        debugMsg = "获取Javascript重定向逻辑"
                        logger.debug(debugMsg)

                if refresh:
                    if kb.alwaysRefresh is None:
                        msg = "获得了一个刷新意图"
                        msg += "类似于登录页面常见的重定向响应）'%s'. " % refresh
                        msg += "你想从现在开始应用它吗? [Y/n]"
            ...
            if conn and not conf.keepAlive:
                try:
                    ...
                    warnMsg = "关闭连接时出现问题 ('%s')" % getSafeExString(ex)
                    logger.warning(warnMsg)
            ...
            if ex.code not in (conf.ignoreCode or []):
                if ex.code == _http_client.UNAUTHORIZED:
                    ...
                    debugMsg = "获取HTTP错误代码: %d ('%s')" % (code, status)
                    logger.debug(debugMsg)
            ...
            if checking:
                ...
                if kb.testMode and kb.testType not in (None, PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED):
                    singleTimeWarnMessage("目标（或WAF/IPS）可能正在丢弃'可疑'请求")
                    kb.droppingRequests = True
                warnMsg = "与目标URL的连接超时"
            elif "Connection reset" in tbMsg:
                if not conf.disablePrecon:
                    singleTimeWarnMessage("由于连接重置，关闭预连接机制")
                    conf.disablePrecon = True

                if kb.testMode:
                    singleTimeWarnMessage("目标（或WAF/IPS）可能正在重置'可疑'请求")
                    kb.droppingRequests = True
                warnMsg = "连接重置为目标URL"
                ...
                if not conf.agent and not conf.randomAgent:
                    warnMsg += ". 尝试使用选项'--User-Agent' "
                    warnMsg += "或开关'--random-Agent'强制HTTPUser-Agent标头"
            elif "IncompleteRead" in tbMsg:
                warnMsg = "从目标URL检索数据时出现 "
                warnMsg += "不完整的读取错误"
            ...
            for function in kb.postprocessFunctions:
                try:
                    ...
                    errMsg = "运行后处理 "
                    errMsg += "函数'%s' ('%s')时出错" % (function.__name__, getSafeExString(ex))
                    raise SqlmapGenericException(errMsg)
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True, disableTampering=False, ignoreSecondOrder=False):
            ...
            if not disableTampering and kb.tamperFunctions:
                for function in kb.tamperFunctions:
                    ...
                    try:
                        ...
                        errMsg = "运行出错  "
                        errMsg += "tamper函数 '%s' ('%s')" % (function.__name__, getSafeExString(ex))
                        raise SqlmapGenericException(errMsg)

                    if not isinstance(payload, six.string_types):
                        errMsg = "tamper 函数 '%s' 返回" % function.__name__
                        errMsg += "无效的负载类型 ('%s')" % type(payload)
                        raise SqlmapValueException(errMsg)
                ...
                if (place in (PLACE.GET, PLACE.URI, PLACE.COOKIE) or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper()) and not conf.skipUrlEncode or place in (PLACE.POST, PLACE.CUSTOM_POST) and postUrlEncode:
                    ...
                    if place == PLACE.COOKIE or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper():
                        if kb.choices.cookieEncode is None:
                            msg = "是否要对cookie值进行URL编码（特定于实现）? %s" % ("[Y/n]" if not conf.url.endswith(".aspx") else "[y/N]")  # Reference: https://support.microsoft.com/en-us/kb/313282
        ...
        if conf.csrfToken:
            ...
            for attempt in xrange(conf.csrfRetries + 1):
                ...
                if attempt > 0:
                    warnMsg = "无法在 '%s'处找到anti-CSRF token '%s'" % (conf.csrfToken._original, conf.csrfUrl or conf.url)
                    warnMsg += ". sqlmap将重试请求"
                    logger.warning(warnMsg)
            ...
            if not token:
                errMsg = "在'%s'处找不到anti-CSRF token'%s'" % (conf.csrfToken._original, conf.csrfUrl or conf.url)
                if not conf.csrfUrl:
                    errMsg += ". 您可以尝试通过为选项 "
                    errMsg += "'--csrf url'提供有效值来重新运行"
        ...
        if timeBasedCompare and not conf.disableStats:
            if len(kb.responseTimes.get(kb.responseTimeMode, [])) < MIN_TIME_RESPONSES:
                ...
                if conf.tor:
                    warnMsg = "由于固有的高延迟时间"
                    warnMsg += "强烈建议避免在基于时间的注入中使用开关'--tor'"
                    singleTimeWarnMessage(warnMsg)

                warnMsg = "[%s] [警告] %s 基于时间的比较需要 " % (time.strftime("%X"), "(case) " if kb.responseTimeMode else "")
                warnMsg += "%s 需要统计模型，请稍候" % ("larger" if len(kb.responseTimes) == 1 else "reset of")
                dataToStdout(warnMsg)
            ...
            if not kb.laggingChecked:
                ...
                if deviation is not None and deviation > WARN_TIME_STDEV:
                    ...
                    warnMsg = " 在连接响应中检测到相当大的滞后"
                    warnMsg += "请为选项"
                    warnMsg += "'--time-sec'使用尽可能高的值 (例如 "
                    warnMsg += "10 or more)"
                    logger.critical(warnMsg)
        ...
        if message:
            ...
            singleTimeWarnMessage("检测到潜在的权限问题 ('%s')" % message)
