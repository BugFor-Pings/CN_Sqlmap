            ...
            for cookie in conf.cj:
                ...
                if ("%s=" % getUnicode(cookie.name)) in getUnicode(headers[HTTP_HEADER.COOKIE]):
                    if conf.loadCookies:
                        ...
                        message = "您提供了HTTP %s 标头值 " % HTTP_HEADER.COOKIE
                        message += "而目标URL提供了它自己的cookie "
                        message += "其中包含与您的标头相交的HTTP %s标头. " % HTTP_HEADER.SET_COOKIE
                        message += "是否要在以后的请求中合并它们? [Y/n] "
    ...
    if contentEncoding in ("gzip", "x-gzip", "deflate"):
        ...
        try:
            ...
            if b"<html" not in page:  # in some cases, invalid "Content-Encoding" appears for plain HTML (should be ignored)
                errMsg = "检测到声明内容编码的 "
                errMsg += "无效数据 '%s' ('%s')" % (contentEncoding, getSafeExString(ex))
            ...
            if identYwaf.non_blind_check(rawResponse, silent=True):
                for waf in set(identYwaf.non_blind):
                    if waf not in kb.identifiedWafs:
                        ...
                        errMsg = "WAF/IPS识别为 '%s'" % identYwaf.format_name(waf)
