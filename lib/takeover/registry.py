...
class Registry(object):
    ...
    def _createRemoteBatchFile(self):
        logger.debug("创建批处理文件 '%s'" % self._batPathRemote)
    ...
    def readRegKey(self, regKey, regValue, parse=False):
        ...
        logger.debug("正在读取注册表项 '%s' 值 '%s'" % (regKey, regValue))
    ...
    def addRegKey(self, regKey, regValue, regType, regData):
        ...
        debugMsg = "添加注册表项值 '%s' " % self._regValue
        debugMsg += "到注册表项 '%s'" % self._regKey
    ...
    def delRegKey(self, regKey, regValue):
        ...
        debugMsg = "删除注册表项值 '%s' " % self._regValue
        debugMsg += "来自注册表项 '%s'" % self._regKey
        logger.debug(debugMsg)
