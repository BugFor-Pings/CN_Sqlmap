...
class XP_cmdshell(object):
    ...
    def _xpCmdshellConfigure2005(self, mode):
        debugMsg = "使用sp_configure存储过程配置 "
        debugMsg += "xp_cmdshell"
    ...
    def _xpCmdshellConfigure2000(self, mode):
        debugMsg = "使用sp_addextendedproc存储过程配置 "
        debugMsg += "xp_cmdshell"
        logger.debug(debugMsg)
    ...
    def _xpCmdshellTest(self):
        ...
        logger.info("测试xp_cmdshell扩展过程是否可用")
        output = self.xpCmdshellEvalCmd("echo 1")

        if output == "1":
            logger.info("xp_cmdshell扩展过程可用")
        elif isNoneValue(output) and conf.dbmsCred:
            errMsg = "似乎临时目录('%s') 用于 " % self.getRemoteTempPath()
            errMsg += "在后端文件系统中存储控制台输出"
            errMsg += "不具有DBMS进程的写入权限. "
            errMsg += "建议您使用选项手动调整 "
            errMsg += "'--tmp-path' 否则您将无法"
            errMsg += "检索命令输出"
            logger.error(errMsg)
    ...
    def xpCmdshellInit(self):
        if not kb.xpCmdshellAvailable:
            ...
            if result:
                ...
                message = "xp_cmdshell扩展过程似乎不可用 "
                message += "是否希望sqlmap尝试 "
                message += "重新启用它？[Y/n] "
                ...
                if readInput(message, default='Y', boolean=True):
                    ...
                    if self._xpCmdshellCheck():
                        ...
                        if self._xpCmdshellCheck():
                            ...
                            warnMsg = "xp_cmdshell创建失败，可能 "
                            warnMsg += "是因为sp_OACreate已禁用"
                            logger.warning(warnMsg)

            hashDBWrite(HASHDB_KEYS.KB_XP_CMDSHELL_AVAILABLE, kb.xpCmdshellAvailable)

            if not kb.xpCmdshellAvailable:
                errMsg = "没有xp_cmdshell无法继续"
                raise SqlmapUnsupportedFeatureException(errMsg)

        debugMsg = "创建支持表以将命令标准输出 "
        debugMsg += "写入"
        logger.debug(debugMsg)
